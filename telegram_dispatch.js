...
function isForFreeUsers(signal, message_data) {
  return api.getSubscriptionTemplate('free').then(templateJson => {
    ...
    var isMediumHorizon = message_data.horizon == 'medium'
    var isPoloniex = message_data.source.ToLowerCase() == 'poloniex'
    return isFreeTicker && isUSDT && isFreeLevel && isUptrend && isMediumHorizon && isPoloniex
  })
}
