...
module.exports = function(bot) {
  ...
  this.cmd = async (msg, params) => {
    ...
    if (!existingUser.eula) sendEula(msg);
    else moduleBot.sendMessage(msg.chat.id, "You already accepted the EULA");
...
function sendEula(msg) {
  ...
  moduleBot
    .sendMessage(chat_id, getEulaText(chat_id), markdown_opts)
    .then(() => {
      moduleBot.sendMessage(
        chat_id,
        "Note: read the EULA to the bottom and click on the link to accept."
      );
    });
}
